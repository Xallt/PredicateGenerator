...
def gen_args(arg, size, pre=None):
    ...
    for pre_size in range(1, size - 2 * (arg - 1)):
        gen_arg += combs(
            [","],
            Term.generate(pre_size, pre),
            gen_args(arg - 1, size - pre_size - 1, pre),
        )
    return gen_arg
...
class UnitFormula:
    ...
    def check_ok(
        ...
        check_x_range=False,
        check_free=False,
        check_double_inv=False,
        check_correct=False,
    ):
        return (
            (UnitFormula.check_x_range(line) or not check_x_range)
            and (not UnitFormula.check_has_free(line) or not check_free)
            and (not UnitFormula.check_double_inv(line) or not check_double_inv)
    ...
    def generate(cls, size, pre=None):
        ...
        for num in range(size):
            for arg in range(size):
                ...
                pre_str = ["F" + "|" * arg + ";" + "|" * num + "("]
                gen += combs(
                    pre_str, gen_args(arg, size - len(pre_str[0]) - 1, pre), [")"]
                )
        return gen
...
class BinAndWrap(WrapPredicate):
    ...
    def parse(cls, line):
        ...
        while i < len(line):
            ...
            if balance == 0 and line[i : i + 3] == "%&%":
                return BinAndWrap(
                    Predicate.parse(line[1:i]), Predicate.parse(line[i + 3 : -1])
                )
            i += 1
        assert False
...
class Evolution:
    ...
    def trans_generate(
        ...
        for i in range(size + 1):
            ...
            preds = Predicate.generate(i, ev)
            ev.terms.append(
                translator.clear_gen(tts, check_untranslatable, check_x_range)
            )
            ev.predicates.append(
                [
                    i
                    for i in translator.clear_gen(
                        preds, check_untranslatable, check_x_range
                    )
                    if UnitFormula.check_ok(
                        i,
                        check_double_inv=check_double_inv,
                        check_x_range=check_x_range,
            ...
            add_predicates = [
                i
                for i in ev.predicates[i]
                if UnitFormula.check_ok(i, check_free=check_free)
            ]
            for pred in add_predicates:
                if do_translation:
                    translated = translator.translate(pred, check_untranslatable)
                    if not (check_untranslatable and "#" in translated) and not (
                        check_x_range
                        and not UnitFormula.check_x_range(str(pred))
                        and not X.check(pred)
                    ):
                        yield translated
                else:
                    yield pred
...
class Translation:
    ...
    def scan(self, interp, arg):
        ...
        if type(arg) in (F, P):
            ...
            return
        raise Exception(
            "Something definitely went wrong with {} and {}".format(interp, arg)
        )
...
class Translator:
    ...
    def translate(self, arg, check_untranslatable=False):
        ...
        if isinstance(arg, (F, P)):
            ...
            return str(
                type(arg)(
                    arg.num, [self.translate(i, check_untranslatable) for i in arg.args]
                )
            )
        elif isinstance(arg, (AWrap, EWrap)):
            return str(
                type(arg)(
                    self.translate(arg.x, check_untranslatable),
                    self.translate(arg.predicate, check_untranslatable),
                )
            )
        elif isinstance(arg, InvWrap):
            ...
            return str(
                type(arg)(
                    self.translate(arg.p1, check_untranslatable),
                    self.translate(arg.p2, check_untranslatable),
                )
            )
        return str(arg)
    ...
    def translate_gen(self, gen, check_untranslatable=False, check_x_range=False):
        ...
        for i in gen:
            ...
            if (check_untranslatable and "#" in t) or (
                check_x_range
                and not UnitFormula.check_x_range(str(i))
                and not X.check(i)
            ):
                continue
            ans.append(str(t))
